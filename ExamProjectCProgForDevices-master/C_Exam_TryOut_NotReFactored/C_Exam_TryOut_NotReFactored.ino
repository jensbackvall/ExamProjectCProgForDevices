...
int speedValue = 0X60;
int lastL = 0;
int innerTrain = 8; // remember to place the correct train on the inner track
int outerTrain = 40; // remember to place the correct train on the outer track
int winnerTrain = 0;
...
int output = 3;
int starttal = 3;
int innerTrackCounter = 0;
int outerTrackCounter = 0;
...
void loop()
{
  rideTwoTrainsIntoTheHorizon(innerTrain, outerTrain);
...
void trainPassed(bool innerTrain){
  ...
  while (passed == false){
    ...
    if (l < 5 && lastL <5) {
        Serial.println("TRAIN ON OUTER TRACK");
        outerTrackCounter = outerTrackCounter + 1;
        passed = true;
    } else if (l > 9 && l < 16 && lastL > 9 && lastL < 16) {
          Serial.println("TRAIN ON INNER TRACK");
          innerTrackCounter = innerTrackCounter + 1;
          passed = true;
    } else {
          Serial.println("NO TRAIN RIGHT NOW");
    }
  ...
  delay(5000);
  if (innerTrackCounter == 5) {
    winnerTrain = innerTrain;
    winnerTrainCircuit(winnerTrain, outerTrackSignals);
    assembleAndSendSignalSwitchBytes(243, 0);
  }
  if (outerTrackCounter == 5) {
    winnerTrain = outerTrain;
    winnerTrainCircuit(winnerTrain, innerTrackSignals);
    assembleAndSendSignalSwitchBytes(251, 0);
    assembleAndSendSignalSwitchBytes(243, 0);
  }
}

void winnerTrainCircuit(int winnerTrain, int signalsArray[8]) {
  int a[8] = {&signalsArray};
  for (int signalAddress: a) {
      assembleAndSendSignalSwitchBytes(signalAddress, 0);
  }
  assembleAndSendSignalSwitchBytes(11, 0);
  assembleAndSendSignalSwitchBytes(51, 1);
  assembleAndSendSignalSwitchBytes(52, 1);
  assembleAndSendSignalSwitchBytes(12, 1);
  assembleAndSendSpeed(0x82, winnerTrain);
  assembleAndSendSpeed(0x82, winnerTrain);
  delay(20000);
  assembleAndSendSpeed(0x84, winnerTrain);
  assembleAndSendSpeed(0x84, winnerTrain);
  delay(20000);
  assembleAndSendSignalSwitchBytes(223, 1);
  assembleAndSendSignalSwitchBytes(224, 1);
  assembleAndSendSignalSwitchBytes(233, 1);
  assembleAndSendSignalSwitchBytes(234, 1);
  assembleAndSendSpeed(0x47, winnerTrain);
  delay(10000);
  assembleAndSendSpeed(0x40, winnerTrain);
}
